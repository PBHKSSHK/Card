-- 自由工作者付款申請可以冇發票 (直接申請表填寫)：
--   供應商 (payee_type <> 'freelancer') — 發票號/發票日期/發票總額照舊必填
--   自由工作者 — 選填；有填發票號先做「同一收款人發票號唔可以重複」查重
...
begin
  ...
  if new.claim_type = 'payment' then
    -- 供應商必須有發票；自由工作者可以直接申請表填寫
    if coalesce(new.payee_type, 'supplier') <> 'freelancer' then
      if new.supplier_invoice_no is null or btrim(new.supplier_invoice_no) = '' then
        raise exception '供應商付款申請必須填發票號';
      end if;
      if new.invoice_date is null then
        raise exception '供應商付款申請必須填發票日期';
      end if;
      if new.invoice_amount is null or new.invoice_amount <= 0 then
        raise exception '供應商付款申請必須填發票總額';
      end if;
    end if;
    -- 有發票號先查重：同一收款人 + 同一發票號 (只計已提交嘅單)
    if new.supplier_invoice_no is not null and btrim(new.supplier_invoice_no) <> '' then
      ...
      if v_dup_no is not null then
        raise exception '重複發票：% 已用咗同一收款人嘅發票號 %', v_dup_no, new.supplier_invoice_no;
      end if;
    end if;
    return new;
  end if;
